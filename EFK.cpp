...
Vector9d vec_zero = Vector9d::Zero();
Matrix9d mtx_zero = Matrix9d::Zero();
class EFK {
    // EFK类型，用于完成卡尔曼滤波
   ...
   private:
    ...
    // 前一最优状态 previous
    Vector9d status_previous = vec_zero;
    Matrix9d Q_previous = mtx_zero;
    // 先验值 prior
    Vector9d status_prior = vec_zero;
    Matrix9d Q_prior = mtx_zero;
    Matrix9d MTX_status_shift_prior = mtx_zero;  // 状态方程中的变换矩阵，由前一最优状态得到先验值
    // 测量值 msr
    Vector4d status_msr = vec_zero;                                             // 测量得到的状态量
    Eigen::Matrix<double, 4, 9> MTX_msr = Eigen::Matrix<double, 4, 9>::Zero();  // 测量矩阵
    // 后验值/修正值 amend
    Vector9d status_amend = vec_zero;
    Matrix9d Q_amend = mtx_zero;
    Eigen::Matrix<double, 9, 4> K_gain = Eigen::Matrix<double, 9, 4>::Zero();  // 卡尔曼增益矩阵
    Eigen::Matrix4d ultra_args_of_K_gain = Eigen::Matrix4d::Zero();            // 卡尔曼增益矩阵中的R，超参数
    // 误差相关
    Matrix9d Q_noise = mtx_zero;        // 协方差噪声
    Vector4d e_msr = Vector4d::Zero();  // 测量误差
    // 函数
...
void EFK::rotate_status_toggle(void) {
    ...
    status_previous[6] = 0;              // theta归零，估计一个新装甲板
    update_MTX_msr(status_previous[6]);
    return;
}
